...
- C에서는 `solution()`만 제출하면 자동 채점됩니다
- 로컬에서 테스트하려면 반드시 `main()`과 `printf("%d\n", ...)` 필요
- 플랫폼은 내부적으로 `main()`을 자동 생성하여 `solution()`을 호출합니다
...
- 특히 C 언어에서는 `printf()` 사용 시 `"%d\n"` 같은 **서식 지정자와 줄바꾸기 처리**를 심각히 신경 쓰여야 하며,\
